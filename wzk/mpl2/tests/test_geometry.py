...
from wzk.mpl2 import geometry, new_fig, DraggableCircle, axes
...
class Test(unittest.TestCase):
    ...
    def test_mpl_all(self):
        ...
        geometry.draw_arc(xy=xy2, radius=r2, ax=ax, theta0=3, theta1=-1, color="r")
        geometry.draw_arc(xy=xy2, radius=r2, ax=ax, theta0=-1, theta1=3, color="b")

        # Basic arc + intersection
        geometry.draw_arc(xy=xy0, radius=r0, ax=ax, alpha=0.7, color="k")
        geometry.draw_arc(xy=xy1, radius=r1, ax=ax, alpha=0.7, color="k")
        (int0, int1), _ = geometry.fill_circle_intersection(xy0=xy0, r0=r0, xy1=xy1, r1=r1, color="y")

        geometry.draw_rays(xy=xy1, radius0=r1*0.1, radius1=r1*0.9, theta0=int1[1], theta1=int1[2],
                           n=100, color="gray")
    ...
    def test_mpl_fill_circle_intersection(self):
        ...
        def update(*args):  # noqa
            geometry.fill_circle_intersection(xy0=circle0.get_xy_drag(), r0=r0,
    ...
    def test_mpl_eye_pov(self):
        ...
        for x in np.arange(5):
            for y in np.arange(5):
                geometry.eye_pov(xy=(x, y), angle=np.random.uniform(0, 2 * np.pi),
    ...
    def test_plot_coordinate_frame(self):
        ...
        h1 = geometry.plot_coordinate_frames(ax=ax, x=[1, 1], dcm=trans_theta2frame(theta=1)[:-1, :-1], color="bb")
        h2 = plot_coordinate_frames(ax=ax, dcm=trans_theta2frame(theta=2)[:-1, :-1], color='ry')  # noqa

        geometry.plot_coordinate_frames(h=h1, dcm=np.eye(3), x=np.ones(3) * 0.1)

        # 3D
        fig, ax = new_fig(aspect=1, title="3D Coordinate Frames")

        axes.set_ax_limits(ax=ax, limits=np.array([[-1, 1],
                                                   [-1, 1],
                                                   [-1, 1]]), n_dim=3)

        dcm = sample_dcm()
        h1 = geometry.plot_coordinate_frames(ax=ax, dcm=dcm)
