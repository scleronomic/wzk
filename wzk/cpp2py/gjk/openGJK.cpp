...
#define eps_rel22        (double) DBL_EPSILON * 1e4f
#define eps_tot22        (double) DBL_EPSILON * 1e2f
...
determinant(const double p[3], const double q[3], const double r[3]) {
  return p[0] * ((q[1] * r[2]) - (r[1] * q[2])) - p[1] * (q[0] * r[2] - r[0] * q[2])
         + p[2] * (q[0] * r[1] - r[0] * q[1]);
}

inline static void
crossProduct(const double a[3], const double b[3], double c[3]) {
  c[0] = a[1] * b[2] - a[2] * b[1];
  c[1] = a[2] * b[0] - a[0] * b[2];
  c[2] = a[0] * b[1] - a[1] * b[0];
}
...
inline static void
projectOnLine(const double p[3], const double q[3], double v[3]) {
  double pq[3];
  pq[0] = p[0] - q[0];
  pq[1] = p[1] - q[1];
  pq[2] = p[2] - q[2];

  const double tmp = dotProduct(p, pq) / dotProduct(pq, pq);
...
inline static void
projectOnPlane(const double p[3], const double q[], const double r[3], double v[3]) {
  ...
  const double tmp = dotProduct(n, p) / dotProduct(n, n);
...
inline static int
hff1(const double p[3], const double q[3]) {
  double tmp = 0;
...
inline static int
hff2(const double p[3], const double q[3], const double r[3]) {
  double ntmp[3];
  double n[3], pq[3], pr[3];
...
inline static int
hff3(const double p[3], const double q[3], const double r[3]) {
  double n[3], pq[3], pr[3];
...
inline static void
S1D(gkSimplex* s, double v[3]) {
  const double* s1p = s->vrtx[1];
  const double* s2p = s->vrtx[0];
...
inline static void
S2D(gkSimplex* s, double v[3]) {
  const double* s1p = s->vrtx[2];
  const double* s2p = s->vrtx[1];
  const double* s3p = s->vrtx[0];
...
S3D(gkSimplex* s, double v[3]) {
  ...
  const double det134 = determinant(s1s3, s1s4, s1s2);
  ...
  switch (testPlaneTwo + testPlaneThree + testPlaneFour) {
    ...
    case 1:
      ...
      if (dotTotal == 1) {
        ...
        // MM : ALL THIS HYPOTHESIS IS FALSE
...
inline static void
support(gkPolytope* body, const double v[3]) {
  double s, maxs;
  double vrt[3];
  int better = -1;
...
inline static void
subalgorithm(gkSimplex* s, double v[3]) {
  switch (s->nvrtx) {
    case 4:
      S3D(s, v);
      break;
...
compute_minimum_distance(gkPolytope* bd1, gkPolytope* bd2, gkSimplex* s) {
  ...
  const double eps_rel = eps_rel22;  /**< Tolerance on relative             */
  const double eps_tot = eps_tot22;  /**< Tolerance on absolute distance    */
