...
from wzk.mpl import (new_fig, save_fig, close_all,
                     set_borders, change_tick_appearance,
...
def reorder(labels, labels_ordered):
    if isinstance(labels, np.ndarray):
        labels = labels.tolist()

    ordering = []
    for lo in labels_ordered:
        ordering.append(labels.index(lo))
    return ordering
...
def value_counts(s, ordering=None):
    ...
    if ordering is not None:
        if isinstance(ordering[0], str):
            ordering = reorder(labels=v, labels_ordered=ordering)
        v = v[ordering]
        c = c[ordering]
...
def set_elongated_ticks(ax, labels, newline, newline_size, y=False):
    ...
    for i, l in enumerate(labels):
        if newline[i]:
            labels[i] = f"\n{l}"
            change_tick_appearance(ax=ax, position=position, v=i, size=newline_size)
...
def mosaic_plot(*, df, col_x, col_y,
                orderings_x=None, orderings_y=None, newline=None, colors, ignore_na=True,
                perc_in_label=False, abs_in_xt=False,
                newline_in_legend=' ', newline_in_label=' ',
                absolut_right=True,
                curly_highlight=None):
    ...
    fig, ax = new_fig(width=plot_size_big[0], height=plot_size_big[1])
    ...
    bottom = np.zeros(n_x)
    temp = np.zeros(n_x)
    bool_x = [df_x == vx for vx in val_x]
    ...
    for i, vy in enumerate(val_y):
        ...
        ax.bar(x_idx, temp, width=width, bottom=bottom,
               label=vy + ' ({:.1f}%)'.format(bool_y.mean()*100) if perc_in_label else vy,
               color=colors[i])

        if curly_highlight is not None and i in curly_highlight[:, 0]:
            jj = curly_highlight[curly_highlight[:, 0] == i, 1]
            for j in jj:

                cb = CurlyBrace(x0=(x_idx[j] - width[j]/3, bottom[j]),
                                x1=(x_idx[j] - width[j]/3, bottom[j]+temp[j]),
                                x2=(x_idx[j] - width[j]/3 + 1, bottom[j]+temp[j]/2),
                                zorder=10)
                ax.add_patch(cb)
    ...
    # Axis Labels
    # x
    labels_xb = [lxb.replace('\n', newline_in_label) for lxb in val_x.tolist()]
    labels_xb = [lxb.replace('.u', 'undecided') for lxb in labels_xb]

    labels_xt = ["{} ({:.1f}%)".format(c, c_perc) if abs_in_xt else '{:.1f}%'.format(c_perc)
    ...
    for i in np.nonzero(newline)[0]:
        change_tick_appearance(ax=ax, position='bottom', v=i, size=newline_size)
        change_tick_appearance(ax=ax_top, position='top', v=i, size=newline_size)
    ...
    ax.set_xlabel(col_x, fontweight=fontweight_label)
    ax.set_ylabel(col_y, fontweight=fontweight_label)

    make_every_box_fancy(ax=ax)
    save_fig(file=f"{fig_directory}/Mosaic/{col_x}__vs__{col_y}", fig=fig, formats=formats)
    close_all()


def bar_plot(df, col, ordering=None, colors=None, x_label='', newline=None,
             orientation='vertical', width=plot_size[0], height=plot_size[1]):

    labels, counts = value_counts(df[col], ordering=ordering)
    bar_height = counts / counts.sum()

    if orientation == 'vertical':
        fig, ax = new_fig(width=width, height=height)

        ax.yaxis.set_major_formatter(ticker.PercentFormatter(1.0, decimals=0))
        ax.bar(np.arange(len(labels)), bar_height, color=list(colors))
        ax.set_xticks(np.arange(len(labels)))
        set_elongated_ticks(ax=ax, labels=labels, newline=newline)

        add_absolute_ax(ax=ax, a=counts.sum(), f=100, position='right')
        ax.set_xlabel(col + x_label, fontweight=fontweight_label)
        save_fig(file=f"{fig_directory}/Bar_ver/{col + x_label}".replace('\n', ', '),
                 fig=fig, formats=formats, bbox=save_bbox)

    elif orientation == 'horizontal':
        set_borders(left=0.2, right=0.87, top=0.9, bottom=0.1)

        fig, ax = new_fig(width=plot_size_square[0], height=plot_size_square[1])

        ax.xaxis.set_major_formatter(ticker.PercentFormatter(1.0, decimals=0))
        ax.barh(np.arange(len(labels)), bar_height, color=colors)
        ax.set_yticks(np.arange(len(labels)))
        set_elongated_ticks(ax=ax, labels=labels, newline=newline, y=True)

        ax.set_ylabel(col + x_label, rotation=-90, fontweight=fontweight_label)
        add_absolute_ax(ax=ax, a=counts.sum(), f=100, position='top')
        save_fig(file=f"{fig_directory}/Bar_hor/{col + x_label}", fig=fig, formats=formats)
    else:
        raise ValueError
...
def multi_bar_plot(df, cols, ordering=None, colors=None, x_label='', newline=None):
    ...
    fig, ax = new_fig(width=plot_size[0], height=plot_size[1])
    ax.yaxis.set_major_formatter(ticker.PercentFormatter(1.0, decimals=0))
    ...
    ax.legend()
    save_fig(file=f"{fig_directory}/Multi_Bar/{cols[0]}__{len(cols)}_{x_label}", fig=fig, formats=formats)
    close_all()


def multi_bar_overlay_plot(df, cols, ordering=None, colors=None, x_label='', newline=None,
    ...
    add_absolute_ax(ax=ax, a=c_max, f=100, position='right')
    ax.legend()
    save_fig(file=f"{fig_directory}/Multi_Bar_Overlay/{cols[0]}__{len(cols)}_{x_label}_sort={sort}_shift{shift}",
             fig=fig, formats=formats)
    close_all()


def multiple_choice_bar(df, cols, ordering=None, colors=None, x_label='', newline=None):
    labels, counts = change_tuple_order((value_counts(df[col], ordering=ordering) for col in cols))

    labels = np.array([l[0] for l in labels])
    counts = np.array([c[0] for c in counts])
    ...
    save_fig(file=f"{fig_directory}/MultipleChoice_Bar/{cols[0]}__{len(cols)}_{x_label}", fig=fig, formats=formats)
    close_all()


def conditional_bar_plot(*, df, col, col_cond, val_cond, ordering=None, colors, newline=None):
    bool_cond = (df[col_cond] == val_cond).values
    val_cond = val_cond.replace('\n', ', ')
    bar_plot(df=df.iloc[bool_cond, :], col=col, colors=colors, newline=newline, ordering=ordering,
             x_label=f"\n(given '{col_cond}'='{val_cond}')")


def pie_plot(df, col, ordering=None, colors=None, autopct='%1.1f%%', newline=None, x_label=''):
    labels, counts = value_counts(df[col], ordering=ordering)

    fig, ax = new_fig(width=plot_size[0], height=plot_size[1])

    patches, hl_o, hl_i = ax.pie(counts, colors=colors, startangle=90, counterclock=False,
                                 autopct=autopct,
    ...
    save_fig(file=f"{fig_directory}/Pie/{col+x_label}", fig=fig, fromats=formats)
    close_all()
